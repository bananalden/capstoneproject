    ...
    $('.edit-btn').on('click', function(){
        ...
        $.ajax({
            ...
            success: function(data){
               ...
               populate_teachers(data.teacher_id)
            },
            error: function(){
...
function populate_teachers(teacherSelected){
    $.ajax({
        url: '/api/get-teacher-list',
        type: 'GET',
        dataType: 'json',
        success: function(data){
            var selectCourse = $('#teacher')
            selectCourse.empty()
            selectCourse.append('<option disabled value="">-----</option>');

            $.each(data, function(index, teacher){
                var isSelected = teacher.id == teacherSelected ? 'selected' : ''
                selectCourse.append('<option value="' + teacher.id + '"' + isSelected + '>' + teacher.first_name + ' ' + teacher.last_name + '</option>')
            })
        }
